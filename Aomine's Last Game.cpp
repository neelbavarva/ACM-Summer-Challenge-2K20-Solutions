...
int N;
vector<int> arr;
void grp(vector<int>* graph,int* curr,int* fin,int a,int b,bool* visited,int node,int curlevel){
	visited[node]=true;
	...
	for(int i=0;i<graph[node].size();i++){
		int k = graph[node][i];
		if(!visited[k]){
			grp(graph,curr,fin,a,b,visited,k,curlevel+1);
		}
	}
}
...
grp(trees,curr,fin,0,0,visited,0,0);
